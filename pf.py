...
def pf(PIPELINE_LENGTH, PIPELINE_SIZE, H, VIDEO_FILE):
                    ...
                    for pipeline_frame_idx in range(1, PIPELINE_LENGTH + 1):
                        if pointline[pipeline_frame_idx] is None:
                            ...
                            if next_point_idx is not None:
                                ...
                                pipeline_observations[pipeline_frame_idx].append(
                                    pointline[pipeline_frame_idx]
                                )
                            # else:
                            #    This is supposed to extrapolate a new point, but it causes a runaway affect since we ignore new object observations
                            #     if prev_point_idx > 0:
